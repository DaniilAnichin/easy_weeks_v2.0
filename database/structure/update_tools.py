...
def update_university(s, id_u=1, full_name='', short_name=''):
    ...
    if exists_un:
        print "This university %s, %s already exist with id %d" % (
            full_name, short_name, exists_un.id
        )
        return -3
...
def update_faculty(s, id_f=1, id_u=1, full_name='', short_name=''):
    ...
    exists_f = s.query(Faculties).filter(
        and_(Faculties.short_name == short_name,
             Faculties.full_name == full_name,
             Faculties.id_university == id_u)).first()
    if exists_f:
        print "This faculty %s, %s already exist in university %s with id %d" % (
            full_name, short_name, exists_f.universities.short_name, exists_f.id
        )
        return -4
...
def update_department(s, id_d=1, id_f=1, full_name='', short_name=''):
    ...
    exists_d = s.query(Departments).filter(
        and_(Departments.short_name == short_name,
             Departments.full_name == full_name,
             Departments.id_faculty == id_f)).first()
    if exists_d:
        print "This department %s, %s already exist in faculty %s with id %d" % (
            full_name, short_name, exists_d.faculty.short_name, exists_d.id
        )
        return -5
...
def update_group(s, id_g=0, id_d=1, name=''):
    ...
    if exists_g:
        print "This group %s already exist in department %s with id %d" % (
            name, exists_g.department.short_name, exists_g.id
        )
        return -7
...
def update_teacher(s, id_t, full_name='', short_name='', id_dep=1, id_deg=1):
    ...
    if exists_te:
        print "Teacher %s %s (%s) from %s is already exists with id %d" % (
            exists_te.degrees.short_name, exists_te.full_name,
            exists_te.short_name, exists_te.departments.short_name,
            exists_te.id
        )
...
def update_room(s, id_r=1, name='', cap=32, stuff='', id_des=[1]):
    ...
    if exist_ro:
        print "Room %s already exists in department %s with id %d" % (
            exist_ro.name, exist_ro.departments.short_name, exist_ro.id
        )
        return -10
...
def update_subject(s, id_s=0, short_name='', full_name=''):
    ...
    if not s.query(Subjects).get(id_s):
        ...
        return -11
    exists_sub = s.query(Subjects).filter(
        and_(Subjects.short_name == short_name,
             Subjects.full_name == full_name)).first()
    if exists_sub:
        print "This subject %s, %s already exist with id %d" % (
            full_name, short_name, exists_sub.id
        )
        return -11
...
def update_lesson_plan(s, id_lp=0, id_sub=0, id_les_type=1, id_grps=[],
    ...
    exist_lp = s.query(LessonPlans).filter(
        LessonPlans.param_checker == param_checker).first()
    if exist_lp:
...
def update_tmp_lesson(s, id_l, id_lp, id_room, row_time):
    ...
    if not s.query(TmpLessons).get(id_l):
        print "no Such lesson with id %d" % id_l
        return -13
    ...
    exist_ls = s.query(TmpLessons).filter(
        TmpLessons.id_lesson_plan == id_lp).all()
    for g in s.query(LessonPlans).get(id_lp).groups:
        for lessons in select_lessons(s, id_lesson_plan=[i['id'] for i in
                                                         select_lesson_plans(s,
                                                                             groups=[
    ...
    if id_room != 1:
        for room_checker in s.query(TmpLessons).filter(
                        TmpLessons.row_time == row_time).all():
            if room_checker.id_room == id_room:
                "This room is busy at this time"
        return -16
    if exist_lp.capacity > s.query(Rooms).get(id_room).capacity:
