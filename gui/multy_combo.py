...
class MultiComboBoxModel(QtGui.QStandardItemModel):
    ...
    def flags(self, index):
        return super(MultiComboBoxModel, self).flags(
            index) | QtCore.Qt.ItemIsUserCheckable
...
class MultiComboBox(QtGui.QComboBox):
    ...
    def clearAll(self):
        ...
        with utils.signalsBlocked(self.model()):
            for row in xrange(self.model().rowCount()):
                self.setItemData(row, QtCore.Qt.Unchecked,
    ...
    def eventFilter(self, receiver, event):
        if event.type() in (QtCore.QEvent.KeyPress, QtCore.QEvent.KeyRelease):
            if receiver == self and event.key() in (
    ...
    def __toggleCheckState(self, index):
        ...
        newValue = QtCore.Qt.Checked if (
        oldValue == QtCore.Qt.Unchecked) else QtCore.Qt.Unchecked
    ...
    def checkedItems(self):
        ...
        """
        if self.model():
            currentIndex = self.model().index(0, self.modelColumn(),
                                              self.rootModelIndex())
            indexes = self.model().match(currentIndex, QtCore.Qt.CheckStateRole,
                                         QtCore.Qt.Checked, -1,
